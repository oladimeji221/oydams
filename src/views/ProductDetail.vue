...
<template>
  <main class="product-page">
    ...
    <section v-if="product" class="product-detail"><div class="detail-image"><img :src="product.image_url" :alt="product.name"/><span>{{ product.available ? product.category : 'Currently unavailable' }}</span></div><div class="detail-copy"><span class="kicker">Made fresh by Oydams</span><h1>{{ product.name }}</h1><p class="detail-description">{{ product.description }}</p><strong class="detail-price">₦{{ Number(product.price).toLocaleString() }}</strong><ul><li><Check/> Freshly prepared</li><li><Check/> Custom requests welcome</li><li><Check/> Delivery confirmed on WhatsApp</li></ul><button class="button primary" @click="add" :disabled="!product.available"><ShoppingBag :size="18"/> {{ product.available ? 'Add to order' : 'Currently unavailable' }}</button><p class="detail-note"><MessageCircle :size="16"/> Final specifications and payment are confirmed directly on WhatsApp.</p></div></section>
